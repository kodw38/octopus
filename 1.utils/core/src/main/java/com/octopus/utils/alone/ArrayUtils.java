...
public class ArrayUtils {
    ...
    public static List subarray(List c,int startIndexInclusive, int endIndexExclusive){
        try{
            List ret = (List)c.getClass().newInstance();
    ...
    public static Map subarray(Map c,int startIndexInclusive, int endIndexExclusive){
        try{
            Map ret = (Map)c.getClass().newInstance();
            Object[] ks = c.keySet().toArray(new Object[0]);
            for(int i=startIndexInclusive;i<endIndexExclusive;i++){
                ret.put(ks[i],c.get(ks[i]));
            }
            return ret;
        }catch (Exception e){
            e.printStackTrace();
    ...
    public static boolean isNotEmpty(Collection array) {
        if(array != null && array.size() != 0){
            ...
            while(its.hasNext()){
                Object o = its.next();
                if(null != o) {
                    if((o instanceof String && StringUtils.isNotBlank((String)o)) || (o instanceof Integer && (Integer)o!=0))
                        return true;
    ...
    public static boolean isInStringArray(String[]ss, String s){
    ...
    public static boolean isIgnoreCaseInStringArray(String[]ss, String s){
        if(null == ss)return false;
        for(int i=0; i<ss.length; i++){
    ...
    public static boolean isContainStringArray(String[]ss, String s){
        if(null == ss)return false;
        for(int i=0; i<ss.length; i++){
            if(ss[i].contains(s))
                return true;
    ...
    public static boolean isLikeStringArray(String[]ss, String s){
        if(null == ss)return false;
        for(int i=0; i<ss.length; i++){
            if(s.indexOf(ss[i])>=0)
                return true;
    ...
    public static boolean isArrayLikeString(String[]ss, String s){
        if(null == ss)return false;
        for(int i=0; i<ss.length; i++){
            if(ss[i].indexOf(s)>=0)
                return true;
    ...
    public static boolean isInStringArray(List ss, String s){
        if(null != ss && null!=s) {
    ...
    public static boolean isInObjectArray(Object[] ss,Object o){
        for(int i=0; i<ss.length; i++){
            if(o instanceof Field && ((Field) o).getName().equals(((Field)ss[i]).getName())){
                return true;
    ...
    public static boolean isInCharArray(char[] ss,char o){
        for(int i=0; i<ss.length; i++){
            if(o==ss[i]){
                return true;
            }
    ...
    public static int indexOfCharArray(char[] ss,char o){
        for(int i=0; i<ss.length; i++){
            if(o==ss[i]){
                return i;
            }
     ...
     * @return
     */
    public static boolean isInIntArray(int[] data,int a){
    ...
    public static boolean isInDoubleArray(double [] data,double a){
        for(int i=0; i<data.length; i++){
            if(data[i]==a)
                return true;
        }
    ...
    public static Object getFirst(Object[] os){
        if(isNotEmpty(os))
            return os[0];
        return null;
    }
    ...
    public static String toString(List os,String splitChar){
        if(null != os){
            StringBuffer sb = new StringBuffer();
            for(Object o:os){
    ...
    public static String toJoinString(Object[] os){
        if(null != os){
            ...
            for(Object o:os){
                if(null != o) {
                    if(o instanceof String) {
                        ...
                        sb.append(ObjectUtils.convertMap2String((Map)o));
                    }else{
                        try {
                            AtomicLong l = new AtomicLong();
                            Map m = POJOUtil.convertPojo2Map(o, l);
                            sb.append(ObjectUtils.convertMap2String(m));
                        }catch (Exception e){
    ...
    public static String toJoinString(long[] os){
        if(null != os){
            StringBuffer sb = new StringBuffer();
            for(Object o:os){
                if(null != o) {
    ...
    public static String toJoinString(Collection os){
        if(null != os){
            StringBuffer sb = new StringBuffer();
            Iterator its = os.iterator();
            while (its.hasNext()){
    ...
    public static String toDistinctString(Collection os){
        if(null != os){
            StringBuffer sb = new StringBuffer();
            Iterator its = os.iterator();
    ...
    public static Object collection(Object[] os){
        try{
            int t=0;
            Class c=null;
            for(Object o:os){
                if(null !=o && o.getClass().isArray()){
                   t=0;
                   break;
                }else if(null != o && o instanceof Collection){
                    t=1;
                    c = o.getClass();
                    break;
                }else if(null !=o && o instanceof Map){
                    t=2;
                    c = o.getClass();
                    break;
                }
    ...
    public static List<String> sortByLen(List<String> li,int type){
        if(type==ABS){
    ...
    //get difficult value from list
    public static List diffList(List ls){
        if(null != ls){
    ...
    //get intersection Map from o1. field Names of o2 in item of o1 values exist
    public static List intersection(List o1,List o2){
        if((null == o1 || o1.size()==0) && (o2==null || o2.size()==0))return null;
        if((null ==o1 || o1.size()==0) && (o2!=null))return o2;
        if(null != o1 && (o2==null || o2.size()==0))return o1;
        if(o1.get(0) instanceof Map && o2.get(0) instanceof Map){
            List<Map> b = o1;
            List<Map> l = o2;
            if(((Map)o1.get(0)).size()<((Map)o2.get(0)).size()){
                b=o2;
                l=o1;
            }
            List ret = new LinkedList();
    ...
    public static boolean isInMapArray(List<Map> l,Map m){
        if(null ==l)return false;
        if(m==null)return false;
        for(Map t:l){
            Iterator its = t.keySet().iterator();
            boolean isin=true;
            while(its.hasNext()){
                Object k = its.next();
                if( (null != t.get(k) && null !=m.get(k) && !t.get(k).equals(m.get(k)))
                        || (null != t.get(k) && null ==m.get(k))
                        ||  (null == t.get(k) && null !=m.get(k)) ){
                    isin=false;
                    break;
                }
    ...
    public static boolean isInMapArrayByFields(List<Map> l,Map m,List<String> fields){
        if(null ==l)return false;
        if(m==null)return false;
        ...
        for(Map t:l){
            boolean isin=true;
            for(String f:fields){
                if( (null != t.get(f) && null !=m.get(f) && !t.get(f).equals(m.get(f)))
                        || (null != t.get(f) && null ==m.get(f))
                        ||  (null == t.get(f) && null !=m.get(f)) ){
                    isin=false;
                    break;
                }
    ...
    /**
     * 获取data数组中，以groupBy属性分组的集合中，某个key值为数组时，数组中的某个值都存在的值
     * @param data
     * @param key
     * @return
     */
    public static List getHasSameArrayValueInListAllMap(List<Map> data,String groupBy,String key){
        Map map = new HashMap();
        for(Map m:data){
            String by = (String)m.get(groupBy);
            Object o = m.get(key);
            if(null != o ){
                if(o instanceof List){
                    for(Object i:(List)o){
                        appendUnExistValueToMapList(map,by,i);
                    }
                    ...
                    appendUnExistValueToMapList(map,by,o);
                }
            }
        }
        ...
        while(its.hasNext()){
            ...
            List li = (List)map.get(k);
            if(ret==null){
                ret =li;
            }else{
                ret = intersection(ret,li);
            }
            if(null == ret){
                return ret;
            }
    ...
    public static void appendUnExistValueToMapList(Map<String,List> m,String key,Object o){
        if(!m.containsKey(key)){
            m.put(key,new ArrayList());
        }
        if(!m.get(key).contains(o)){
            m.get(key).add(o);
        }
    }
    public static boolean contains(Map all,Map item){
        Iterator its = item.keySet().iterator();
        while(its.hasNext()){
            Object o = its.next();
            if(!(null != all.get(o) && null != item.get(o) && all.get(o).equals(item.get(o)))){
                return false;
            }
        }
        return true;
    }
    public static Map<String,Map> mapList2MapMap(List<Map> src,String Key1,String Keys2,String Value2){
        if(null != src){
            HashMap ret=  new HashMap();
            for(Map m:src){
                if(m.containsKey(Key1)){
                    if(ret.containsKey(m.get(Key1))){
                        ((Map)ret.get(m.get(Key1))).put(m.get(Keys2),m.get(Value2));
                    }else{
                        HashMap t = new HashMap();
                        t.put(m.get(Keys2),m.get(Value2));
                        ret.put(m.get(Key1),t);
                    }
                }
            }
            return ret;
    ...
    public static List innerList(List a,List b){
        if(null != a && null != b){
            List min=null;
            List max;
            if(a.size()<=b.size()){
