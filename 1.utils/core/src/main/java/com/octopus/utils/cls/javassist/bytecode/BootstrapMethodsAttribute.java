...
public class BootstrapMethodsAttribute extends com.octopus.utils.cls.javassist.bytecode.AttributeInfo {
    /**
    ...
    BootstrapMethodsAttribute(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, int n, DataInputStream in)
        throws IOException
    {
        super(cp, n, in);
    }
     ...
     */
    public BootstrapMethodsAttribute(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, BootstrapMethod[] methods) {
        super(cp, tag);
        int size = 2;
        ...
        com.octopus.utils.cls.javassist.bytecode.ByteArray.write16bit(methods.length, data, 0);    // num_bootstrap_methods
        int pos = 2;
        for (int i = 0; i < methods.length; i++) {
            com.octopus.utils.cls.javassist.bytecode.ByteArray.write16bit(methods[i].methodRef, data, pos);
            com.octopus.utils.cls.javassist.bytecode.ByteArray.write16bit(methods[i].arguments.length, data, pos + 2);
            int[] args = methods[i].arguments;
            pos += 4;
            for (int k = 0; k < args.length; k++) {
                com.octopus.utils.cls.javassist.bytecode.ByteArray.write16bit(args[k], data, pos);
                pos += 2;
            }
        }
    ...
    public BootstrapMethod[] getMethods() {
        byte[] data = this.get();
        int num = com.octopus.utils.cls.javassist.bytecode.ByteArray.readU16bit(data, 0);
        BootstrapMethod[] methods = new BootstrapMethod[num];
        int pos = 2;
        for (int i = 0; i < num; i++) {
            int ref = com.octopus.utils.cls.javassist.bytecode.ByteArray.readU16bit(data, pos);
            int len = com.octopus.utils.cls.javassist.bytecode.ByteArray.readU16bit(data, pos + 2);
            int[] args = new int[len];
            pos += 4;
            for (int k = 0; k < len; k++) {
                args[k] = com.octopus.utils.cls.javassist.bytecode.ByteArray.readU16bit(data, pos);
                pos += 2;
            }
