 ...
 */
class CodeAnalyzer implements com.octopus.utils.cls.javassist.bytecode.Opcode {
    private ConstPool constPool;
    private com.octopus.utils.cls.javassist.bytecode.CodeAttribute codeAttr;

    public CodeAnalyzer(com.octopus.utils.cls.javassist.bytecode.CodeAttribute ca) {
        codeAttr = ca;
        ...
        throws com.octopus.utils.cls.javassist.bytecode.BadBytecode
    {
         ...
         */
        com.octopus.utils.cls.javassist.bytecode.CodeIterator ci = codeAttr.iterator();
    ...
    private void visitBytecode(com.octopus.utils.cls.javassist.bytecode.CodeIterator ci, int[] stack, int index)
        throws com.octopus.utils.cls.javassist.bytecode.BadBytecode
    {
        ...
        while (ci.hasNext()) {
            ...
            if (stackDepth < 1)
                throw new com.octopus.utils.cls.javassist.bytecode.BadBytecode("stack underflow at " + index);
    ...
    private boolean processBranch(int opcode, com.octopus.utils.cls.javassist.bytecode.CodeIterator ci, int index,
        ...
        if ((IFEQ <= opcode && opcode <= IF_ACMPNE)
            ...
            case JSR_W :
                ...
                if (jsrDepth[0] < 0) {
                    ...
                    throw new com.octopus.utils.cls.javassist.bytecode.BadBytecode(
        ...
        throws com.octopus.utils.cls.javassist.bytecode.BadBytecode
    {
        if (target < 0 || codeLength <= target)
            throw new com.octopus.utils.cls.javassist.bytecode.BadBytecode("bad branch offset at " + opIndex);

        int d = stack[target];
        if (d == 0)
            stack[target] = -stackDepth;
        else if (d != stackDepth && d != -stackDepth)
            throw new com.octopus.utils.cls.javassist.bytecode.BadBytecode("verification error (" + stackDepth +
                                  "," + d + ") at " + opIndex);
    }
     ...
     */
    private int visitInst(int op, com.octopus.utils.cls.javassist.bytecode.CodeIterator ci, int index, int stack)
        throws BadBytecode
    {
        String desc;
        switch (op) {
        ...
        case INVOKESPECIAL :
            ...
            stack += com.octopus.utils.cls.javassist.bytecode.Descriptor.dataSize(desc) - 1;
            break;
        case INVOKESTATIC :
            desc = constPool.getMethodrefType(ci.u16bitAt(index + 1));
            stack += com.octopus.utils.cls.javassist.bytecode.Descriptor.dataSize(desc);
            break;
        ...
        case INVOKEDYNAMIC :
            ...
            stack += com.octopus.utils.cls.javassist.bytecode.Descriptor.dataSize(desc);     // assume CosntPool#REF_invokeStatic
            break;
        case ATHROW :
