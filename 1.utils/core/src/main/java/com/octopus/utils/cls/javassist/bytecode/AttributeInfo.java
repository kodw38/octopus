...
public class AttributeInfo {
    protected com.octopus.utils.cls.javassist.bytecode.ConstPool constPool;
    int name;
    byte[] info;

    protected AttributeInfo(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, int attrname, byte[] attrinfo) {
        constPool = cp;
        name = attrname;
        info = attrinfo;
    }

    protected AttributeInfo(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, String attrname) {
     ...
     */
    public AttributeInfo(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, String attrname, byte[] attrinfo) {
        this(cp, cp.addUtf8Info(attrname), attrinfo);
    }

    protected AttributeInfo(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, int n, DataInputStream in)
        throws IOException
    {
        constPool = cp;
        name = n;
    ...
    static AttributeInfo read(com.octopus.utils.cls.javassist.bytecode.ConstPool cp, DataInputStream in)
        throws IOException
    {
        ...
        if (nameStr.charAt(0) < 'L') {
            ...
            else if (nameStr.equals(com.octopus.utils.cls.javassist.bytecode.CodeAttribute.tag))
                return new com.octopus.utils.cls.javassist.bytecode.CodeAttribute(cp, name, in);
            ...
            else if (nameStr.equals(com.octopus.utils.cls.javassist.bytecode.LocalVariableAttribute.tag))
            ...
            else if (nameStr.equals(com.octopus.utils.cls.javassist.bytecode.SignatureAttribute.tag))
                return new SignatureAttribute(cp, name, in);
            else if (nameStr.equals(com.octopus.utils.cls.javassist.bytecode.SourceFileAttribute.tag))
     ...
     */
    public com.octopus.utils.cls.javassist.bytecode.ConstPool getConstPool() { return constPool; }
     ...
     *                          class names.
     */
    public AttributeInfo copy(com.octopus.utils.cls.javassist.bytecode.ConstPool newCp, Map classnames) {
        int s = info.length;
        byte[] srcInfo = info;
