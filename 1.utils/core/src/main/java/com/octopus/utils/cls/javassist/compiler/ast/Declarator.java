...
public class Declarator extends ASTList implements TokenId {
    ...
    public Declarator make(Symbol sym, int dim, com.octopus.utils.cls.javassist.compiler.ast.ASTree init) {
    ...
    public com.octopus.utils.cls.javassist.compiler.ast.ASTree getInitializer() {
        ASTList t = tail();
        if (t != null)
            return t.head();
        else
    ...
    private static void astToClassName(StringBuffer sbuf, ASTList name,
                                       char sep) {
        for (;;) {
            com.octopus.utils.cls.javassist.compiler.ast.ASTree h = name.head();
