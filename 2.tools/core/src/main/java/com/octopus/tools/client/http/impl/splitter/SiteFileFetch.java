...
public class SiteFileFetch  implements Runnable {
    static transient Log log = LogFactory.getLog(SiteFileFetch.class);
    SiteInfoBean siteInfoBean = null; //文件信息
    long[] nStartPos; //开始位置
    long[] nEndPos; //结束位置
    ...
    //File tmpInfoFile; //文件下载的临时信息
    DataOutputStream output; //输出到文件的输出流

    public SiteFileFetch(SiteInfoBean bean) throws IOException {
        siteInfoBean = bean;//
        //tmpFile = File.createTempFile ("temp","info",new File(bean.getSFilePath()));
        tmpFile = new File(bean.getSFilePath()+File.separator + bean.getSFileName()+".info");
        if(tmpFile.exists ()){
            bFirst = false;
            read_nPos();
        }else{nStartPos = new long[bean.getNSplitter()];
            nEndPos = new long[bean.getNSplitter()];
        }
    }
    public void run(){//获得文件长度//分割文件//实例FileSplitterFetch//启动FileSplitterFetch线程//等待子线程返回
        try{
            if(bFirst){
            ...
            tmpFile.delete();
            log.info("down finished:"+siteInfoBean.getSSiteURL());
        }catch(Exception e){
            e.printStackTrace ();
        }
    }
